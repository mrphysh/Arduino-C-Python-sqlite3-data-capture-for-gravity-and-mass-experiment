...
data_source_database =  "007_mar_2_2019_database"
summary_database = "october_summary_database"
##
summary_table =  "summary"###this is not working  prelim_summary  and final_summary
##
...
b = bonn.cursor()
b.execute("CREATE TABLE IF NOT EXISTS final_summary (column_zero TEXT  ,  column_one REAL,\
...
def dynamic_data_entry_averages():
    b.execute("INSERT INTO  final_summary(column_zero,column_one,column_two,column_three,\
...
six_target = 13
#
window      = 1200  #  and this is apples and oranges example 1870000 plus or minus 400
#
search_number =  5000
#
print()
...
while (var_one  ):
    ...
    two_average = 0; thr_average = 0; for_average = 0;  five_average=0; six_average = 0
    for row in c.fetchmany(search_number):
        var_one = row[0]
        for i in range (0,5):
            if row[i] != None and type(row[i]) == (float):
                five_var =  (row[i])
...
#     line seems to be required             okaaay
                ...
                if ((five_var)>(five_target*1000)- window  and (five_var)< ((five_target*1000)+ window)):
